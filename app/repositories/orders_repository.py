...
class OrderRepository:
    ...
    def delete_order(self, order_nmber: int):
        temp_list = self._get_orders_list()
        for idx, order in enumerate(temp_list):
            if order_nmber == order['order_n']:
                del temp_list[idx]
                self._push_orders_list(temp_list)
                return True
        raise KeyError('PEDIDOREPOSITORY_DELETAR: Pedido não encontrado.')

    def replace_order(self, old_order: Order, new_order: Order=None):
        if new_order is None:
            new_order = old_order

        self.delete_order(old_order.order_n)
    ...
    def find_order(self, order_nmber: int) -> Order:
        order_nmber = check_and_convert_number_to_float(order_nmber)
        ...
        for order in temp_list:
            if int(order_nmber) == order['order_n']:
